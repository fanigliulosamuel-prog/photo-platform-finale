...
export default function AdminPage() {
  ...
  useEffect(() => {
    // 1. Ripristina l'ultima tab visitata (Così se torni indietro non perdi il segno)
    const savedTab = sessionStorage.getItem('admin_active_tab');
    if (savedTab === 'users' || savedTab === 'photos') {
      setActiveTab(savedTab);
    }

    // 2. Controllo se hai già fatto login in questa sessione
  ...
  // Funzione per cambiare tab e salvarla in memoria
  const changeTab = (tab: 'users' | 'photos') => {
      setActiveTab(tab);
  ...
  function logoutAdmin() {
      ...
      sessionStorage.removeItem('admin_active_tab'); // Pulisce anche la memoria della tab
    ...
    <main className="min-h-screen bg-gradient-to-br from-stone-500 via-stone-600 to-stone-500 text-white p-8 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-8">
            <div className="flex gap-4">
                <button onClick={() => changeTab('photos')} className={`px-6 py-2 rounded-xl font-bold transition shadow-md ${activeTab === 'photos' ? 'bg-red-700 text-white' : 'bg-stone-400/30 text-stone-100 hover:bg-stone-400/50'}`}>Foto ({photos.length})</button>
                <button onClick={() => changeTab('users')} className={`px-6 py-2 rounded-xl font-bold transition shadow-md ${activeTab === 'users' ? 'bg-red-700 text-white' : 'bg-stone-400/30 text-stone-100 hover:bg-stone-400/50'}`}>Utenti ({users.length})</button>
            </div>
                    ...
                    {photos.map(photo => (
                        <div key={photo.id} className="relative group aspect-square bg-stone-800 rounded-lg overflow-hidden border border-stone-500/30 shadow-md">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-stone-900/80 opacity-0 group-hover:opacity-100 transition p-2 text-center">
                                ...
                                {/* Link per vedere la foto reale - Apre stessa scheda con parametro */}
