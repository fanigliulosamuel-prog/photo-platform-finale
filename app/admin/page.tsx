    ...
    <main className="min-h-screen bg-stone-900 text-white flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-stone-800 border border-red-900/50 p-8 rounded-2xl shadow-2xl text-center">
            ...
            <form onSubmit={handleAdminLogin} className="space-y-4">
                <input type="password" placeholder="Codice Segreto" value={secretInput} onChange={(e) => setSecretInput(e.target.value)} className="w-full bg-black/50 border border-stone-600 rounded-lg p-3 text-white focus:border-red-500 outline-none text-center tracking-widest transition"/>
                {loginError && <p className="text-red-400 text-sm font-bold animate-pulse">⛔ Codice Errato.</p>}
                <button className="w-full py-3 bg-red-700 hover:bg-red-600 text-white font-bold rounded-lg transition transform hover:scale-[1.02]">Accedi</button>
            </form>
            <Link href="/dashboard" className="block mt-8 text-stone-500 text-sm hover:text-white transition">← Torna alla Dashboard</Link>
        </div>
    </main>
  );
    ...
    <main className="min-h-screen bg-gradient-to-br from-stone-500 via-stone-600 to-stone-500 text-white p-8 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
            <div><h1 className={`${playfair.className} text-4xl font-bold text-white`}>Pannello Admin</h1><p className="text-stone-200 text-sm">Accesso Segreto Attivo</p></div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-8">
            ...
            {activeTab === 'users' && (
                <div className="relative w-full md:w-64">
                    <input type="text" placeholder="Cerca utente..." value={userSearchTerm} onChange={(e) => setUserSearchTerm(e.target.value)} className="w-full px-4 py-2 bg-stone-800/50 border border-stone-600 rounded-xl text-white placeholder-stone-400 focus:outline-none focus:border-red-500/50 transition text-sm"/>
                    ...
                    {photos.map(photo => (
                        <div key={photo.id} className="relative group aspect-square bg-stone-800 rounded-lg overflow-hidden border border-stone-500/30 shadow-md">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-stone-900/80 opacity-0 group-hover:opacity-100 transition p-2 text-center">
                                ...
                                <Link href={`/photo/${photo.id}?from=admin`} className="mb-2">
                        ...
                        <div key={user.id} className="flex justify-between items-center bg-stone-600/30 p-4 rounded-xl border border-stone-500/30 hover:bg-stone-600/50 transition">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    {/* LINK AL PROFILO: Passa il parametro ?from=admin */}
                                    <Link href={`/profile/${user.username}?from=admin`} className="text-xs bg-stone-500 hover:bg-stone-400 text-white px-3 py-1 rounded-lg font-bold transition flex items-center gap-1">
                                        Vedi Profilo ↗
                                    </Link>
                                </div>
                                <p className="text-xs text-stone-400 font-mono">ID: {user.id}</p>
                            </div>
                            <button onClick={() => deleteUser(user)} className="bg-red-600/20 text-red-400 border border-red-600/50 px-4 py-2 rounded-lg text-xs font-bold hover:bg-red-600 hover:text-white transition whitespace-nowrap ml-4">ELIMINA UTENTE</button>
                        </div>
                    ))}
