...
export default function PhotoDetailPage() {
  ...
  const [commentAuthor, setCommentAuthor] = useState(""); // Nome per il commento
  
  const [loading, setLoading] = useState(true);
  const [hasVoted, setHasVoted] = useState(false); 
  const [userId, setUserId] = useState<string | null>(null);
  const [currentUsername, setCurrentUsername] = useState(""); // Il TUO nome utente (per la notifica)
  ...
  async function sendNotification(type: 'like' | 'comment', message: string) {
    ...
    // 1. Trova l'ID del proprietario della foto (a chi mandare la notifica)
    const { data: ownerProfile } = await supabase
        .from('profiles')
        .select('id')
        .eq('username', photo.author_name) // Cerca l'autore tramite il nome salvato nella foto
        .single();

    // 2. Se trovato e NON sei tu (non mandarti notifiche da solo), spedisci
  ...
  async function handleLike() {
    ...
    try {
      ...
      // B. Incrementa contatore
      const { error: rpcError } = await supabase.rpc('increment_likes', { photo_id_input: photoId });
      if (rpcError) {
          // Fallback manuale se RPC fallisce
  ...
  async function handlePostComment(e: React.FormEvent) {
    ...
    if (!error) {
      // INVIA NOTIFICA ANCHE PER IL COMMENTO
        ...
        <div className="w-full md:w-96 flex flex-col gap-6">
          ...
          <div className="bg-black/20 p-6 rounded-3xl border border-white/5">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              💬 Critiche & Pareri <span className="bg-white/10 text-xs px-2 py-1 rounded-full">{comments.length}</span>
            </h3>
            <div className="flex flex-col gap-4 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {comments.length === 0 ? <p className="text-gray-500 text-sm italic">Nessun commento ancora.</p> : 
                comments.map((c) => (
