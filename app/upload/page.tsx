...
export default function UploadPage() {
  ...
  // 1. Controlla se c'è una categoria nell'URL (es. dalla pagina Sfide)
  useEffect(() => {
  ...
  async function handleUpload(e: FormEvent) {
    ...
    try {
      ...
      // 3. Conversione sicura ISO
      let safeIso: number | null = null;
      if (iso && !isNaN(parseInt(iso))) {
        safeIso = parseInt(iso);
      }
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto h-screen relative z-10 flex flex-col items-center justify-center">
            ...
            <form onSubmit={handleUpload} className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div><label className="block text-xs text-stone-300 mb-1">Camera</label><input type="text" value={camera} onChange={(e) => setCamera(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. Canon R6"/></div>
                    <div><label className="block text-xs text-stone-300 mb-1">Tempo</label><input type="text" value={shutterSpeed} onChange={(e) => setShutterSpeed(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. 1/125s"/></div>
                    <div><label className="block text-xs text-stone-300 mb-1">ISO</label><input type="number" value={iso} onChange={(e) => setIso(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. 400"/></div>
                    <div><label className="block text-xs text-stone-300 mb-1">Apertura</label><input type="text" value={aperture} onChange={(e) => setAperture(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. f/4.0"/></div>
                </div>
                </div>
