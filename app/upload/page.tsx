...
function UploadContent() {
  ...
  async function handleUpload(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Nota: getPublicUrl è sincrono nella v2
      ...
      // Conversione sicura per ISO (gestione numeri)
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-stone-500 via-stone-600 to-stone-500 text-white relative overflow-hidden">
      ...
      <div className="flex w-full relative z-10 h-screen">
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto h-screen relative z-10 flex flex-col items-center justify-center">
            ...
            <div className="relative z-10 w-full max-w-2xl bg-stone-400/40 backdrop-blur-xl border border-stone-300/50 p-8 rounded-3xl shadow-2xl mt-10 md:mt-0">
            <div className="flex justify-center items-center mb-2"><h1 className="text-3xl md:text-4xl font-bold text-white drop-shadow-md text-center">Carica il tuo Scatto</h1></div>
            <p className="text-stone-100 mb-8 text-sm text-center">Mostra il tuo talento alla community.</p>

            <form onSubmit={handleUpload} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-xs font-bold text-stone-200 uppercase tracking-wider mb-2">Titolo Opera</label><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none placeholder-stone-300" placeholder="Titolo" required/></div>
                <div><label className="block text-xs font-bold text-stone-200 uppercase tracking-wider mb-2">Nome Fotografo</label><input type="text" value={author} disabled className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-stone-300 cursor-not-allowed"/></div>
                <div className="md:col-span-2"><label className="block text-xs font-bold text-stone-200 uppercase tracking-wider mb-2">Categoria</label><div className="relative"><select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none appearance-none cursor-pointer">{CATEGORIES.map(cat => <option key={cat} value={cat} className="bg-stone-800">{cat}</option>)}</select><div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-stone-200">▼</div></div></div>
                </div>
                <div className="pt-4 border-t border-stone-400/30"><h3 className="text-sm font-bold text-amber-200 uppercase tracking-wider mb-4">Dati Tecnici (EXIF)</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><div><label className="block text-xs text-stone-300 mb-1">Camera</label><input type="text" value={camera} onChange={(e) => setCamera(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. Canon R6"/></div><div><label className="block text-xs text-stone-300 mb-1">Tempo</label><input type="text" value={shutterSpeed} onChange={(e) => setShutterSpeed(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. 1/125s"/></div><div><label className="block text-xs text-stone-300 mb-1">ISO</label><input type="number" value={iso} onChange={(e) => setIso(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. 400"/></div><div><label className="block text-xs text-stone-300 mb-1">Apertura</label><input type="text" value={aperture} onChange={(e) => setAperture(e.target.value)} className="w-full bg-stone-600/50 border border-stone-400/50 rounded-xl p-3 text-white focus:border-amber-400/50 outline-none" placeholder="Es. f/4.0"/></div></div></div>
                <div className="border-2 border-dashed border-stone-400/50 rounded-2xl p-8 text-center hover:border-amber-400/50 hover:bg-stone-400/20 transition cursor-pointer relative group mt-6"><input type="file" accept="image/*" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" /><div className="relative z-10">{file ? <><p className="text-amber-300 font-bold text-lg mb-1">✅ {file.name}</p></> : <><p className="text-4xl mb-3">📤</p><p className="text-stone-200 font-medium">Clicca o trascina qui</p></>}</div></div>
                <button disabled={uploading} className="w-full py-4 bg-stone-100 text-stone-900 font-bold rounded-xl hover:scale-[1.02] hover:shadow-lg transition transform disabled:opacity-50 mt-6">{uploading ? "Caricamento..." : "Pubblica Foto 🚀"}</button>
            </form>
            </div>
        </main>
      </div>
